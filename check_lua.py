...
DamageTemplet_Path = [join(ScriptBase_Path, f) for f in listdir(ScriptBase_Path) if isfile(join(ScriptBase_Path, f) ) and "LUA_DAMAGE_TEMPLET" in f and splitext(f)[1] == ".txt"]
...
for path in UnitScript_Path:
	ExtractUnitScript(path,"Unit")
print("UnitScript ExtractDone.")

print("Extract Count :", len(ShipScript_Path))
for path in ShipScript_Path:
	ExtractUnitScript(path,"Ship")
print("ShipScript ExtractDone.")

print("Extract Count :", len(MobScript_Path))
for path in MobScript_Path:
	ExtractUnitScript(path,"Mob")
